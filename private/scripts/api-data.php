...
try {
  ...
  die(json_encode(array(
    'users' => array(
      ...
      'perWeek' => getKeyMap('SELECT week AS key, SUM(new) AS new, SUM(existing) AS existing, SUM(new) + SUM(existing) AS total FROM (SELECT STRFTIME(\'%Y-%W\',time_received,\'unixepoch\',\'weekday 0\',\'-6 day\') AS week, device_id, MAX(time_lastPing) == 0 AS new, MAX(time_lastPing) > 0 AS existing FROM ping WHERE time_received > 1464034000 AND device_platform_os = \'android\' AND device_emulator = 0 GROUP BY 1,2) GROUP BY 1 ORDER BY 1 DESC;', true),
    ...
    'drugs' => array(
      ...
      'perWeek' => getKeysMap('SELECT STRFTIME(\'%Y-%W\',v.time_day,\'unixepoch\',\'weekday 0\',\'-6 day\') AS key0, v.what AS key1, SUM(v.num) AS visits, COUNT(DISTINCT p.device_id) AS users FROM visited v JOIN ping p ON v.uuid = p.uuid WHERE v.type = \'drug\' AND v.time_day > 0 AND p.time_received > 1464034000 AND v.time_day >= 1464034000 AND p.device_platform_os = \'android\' AND device_emulator = 0 GROUP BY 1,2 ORDER BY 1 ASC, 3 DESC, 4 DESC, 2 ASC;', true),
    ),
    'devices' => array(
