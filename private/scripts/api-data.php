...
try {
  ...
  die(json_encode(array(
    ...
    'countries' => getKeyValue('SELECT countries AS key, COUNT(*) AS value FROM (SELECT device_id, GROUP_CONCAT(DISTINCT network_country) AS countries FROM ping WHERE time_received > 1464034000 AND device_platform_os = \'android\' AND device_emulator = 0 GROUP BY 1) GROUP BY 1 ORDER BY 2 DESC LIMIT 20;'),
    'drugs' => array(
      'totals' => getKeyMap('SELECT v.what AS key, SUM(v.num) AS visits, COUNT(DISTINCT device_id) AS users FROM visited v JOIN ping p ON v.uuid = p.uuid WHERE v.type = \'drug\' AND p.time_received > 1464034000 AND p.device_platform_os = \'android\' AND device_emulator = 0 GROUP BY 1 ORDER BY 2 DESC, 3 DESC;', true),
      'perWeek' => getKeysMap('SELECT STRFTIME(\'%Y-%W\',v.time_day,\'unixepoch\') AS key0, v.what AS key1, SUM(v.num) AS visits, COUNT(DISTINCT p.device_id) AS users FROM visited v JOIN ping p ON v.uuid = p.uuid WHERE v.type = \'drug\' AND p.time_received > 1464034000 AND p.device_platform_os = \'android\' AND device_emulator = 0 GROUP BY 1,2 ORDER BY 1 DESC, 2 DESC, 3 DESC, 4 DESC;', true),
    ),
    'devices' => array(
      'manufacturer' => getKeyValue('SELECT device_manufacturer AS key, COUNT(distinct device_id) AS value FROM ping WHERE time_received > 1464034000 AND device_platform_os = \'android\' AND device_emulator = 0 GROUP BY 1 ORDER BY 2 DESC LIMIT 20;'),
      'screenSize' => getKeyMap('SELECT device_screen_width || \'x\' || device_screen_height || \'@\' || device_screen_scale AS key, device_screen_width AS relWidth, device_screen_height AS relWeight, CAST(device_screen_width * device_screen_height AS INT) AS relPixels, device_screen_scale AS scale, CAST(device_screen_width * device_screen_scale AS INT) AS natWidth, CAST(device_screen_height * device_screen_scale AS INT) AS natHeight, CAST(device_screen_width * device_screen_scale * device_screen_height * device_screen_scale AS INT) AS natPixels, ROUND(CAST(100 * device_screen_height AS FLOAT) / CAST(100 * device_screen_width AS FLOAT),2) AS aspect, CAST(device_screen_width * device_screen_height AS INT) >= (480 * 640) AS isTablet, COUNT(DISTINCT device_id) AS devices FROM ping WHERE time_received > 1464034000 AND device_platform_os = \'android\' AND device_emulator = 0 GROUP BY 1 ORDER BY 4 DESC, 5 DESC;', true),
    ),
  )));
