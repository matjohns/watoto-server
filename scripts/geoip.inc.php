...
function _geoip_load_shared_mem($file)
{
    $fp = fopen($file, "rb");
    if (!$fp) {
...
function _geoip_open($filename, $flags)
{
    $gi = new GeoIP;
    $gi->flags = $flags;
    if ($gi->flags & GEOIP_SHARED_MEMORY) {
...
function _geoip_country_id_by_name_v6($gi, $name)
{
    ...
    return _geoip_country_id_by_addr_v6($gi, $addr);
}

function _geoip_country_id_by_name($gi, $name)
{
    $addr = gethostbyname($name);
    if (!$addr || $addr == $name) {
        return false;
    }
    return _geoip_country_id_by_addr($gi, $addr);
}

function _geoip_country_code_by_name_v6($gi, $name)
{
    $country_id = _geoip_country_id_by_name_v6($gi, $name);
    if ($country_id !== false) {
...
function _geoip_country_code_by_name($gi, $name)
{
    $country_id = _geoip_country_id_by_name($gi, $name);
    if ($country_id !== false) {
        return $gi->GEOIP_COUNTRY_CODES[$country_id];
    }
    return false;
...
function _geoip_country_name_by_name_v6($gi, $name)
{
    $country_id = _geoip_country_id_by_name_v6($gi, $name);
    if ($country_id !== false) {
...
function _geoip_country_id_by_addr_v6($gi, $addr)
{
    $ipnum = inet_pton($addr);
    return _geoip_seek_country_v6($gi, $ipnum) - GEOIP_COUNTRY_BEGIN;
}

function _geoip_country_id_by_addr($gi, $addr)
{
    $ipnum = ip2long($addr);
    return _geoip_seek_country($gi, $ipnum) - GEOIP_COUNTRY_BEGIN;
}

function _geoip_country_code_by_addr_v6($gi, $addr)
{
    $country_id = _geoip_country_id_by_addr_v6($gi, $addr);
    if ($country_id !== false) {
        return $gi->GEOIP_COUNTRY_CODES[$country_id];
    }
    return false;
...
function _geoip_country_code_by_addr($gi, $addr)
{
    if ($gi->databaseType == GEOIP_CITY_EDITION_REV1) {
        $record = _geoip_record_by_addr($gi, $addr);
        if ($record) {
            return $record->country_code;
        }
    } else {
        $country_id = _geoip_country_id_by_addr($gi, $addr);
        if ($country_id !== false) {
            return $gi->GEOIP_COUNTRY_CODES[$country_id];
        }
    }
...
function _geoip_country_name_by_addr_v6($gi, $addr)
{
    $country_id = _geoip_country_id_by_addr_v6($gi, $addr);
    if ($country_id !== false) {
        return $gi->GEOIP_COUNTRY_NAMES[$country_id];
...
function _geoip_country_name_by_addr($gi, $addr)
{
    if ($gi->databaseType == GEOIP_CITY_EDITION_REV1) {
        $record = _geoip_record_by_addr($gi, $addr);
...
function _geoip_name_by_addr_v6($gi, $addr)
{
    if ($addr == null) {
        return 0;
    }
...
function _geoip_name_by_addr($gi, $addr)
{
    if ($addr == null) {
        return 0;
    }
...
function _geoip_org_by_addr($gi, $addr)
{
    return _geoip_name_by_addr($gi, $addr);
}
...
function _geoip_region_by_addr($gi, $addr)
{
    if ($addr == null) {
        return 0;
    }
